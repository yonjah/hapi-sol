...
implementation = function (server, options) {
	...
	hoek.assert(settings.sidLength > 0, 'sidLength must be a positive Integer');
	...
	if (!settings.assumePromise || !settings.cache) {
		cacheGet  = Promise.promisify(cacheGet);
					...
					.spread(function (session/*, cached*/) {
						...
						return settings
							...
							.spread(function (isValid, credentials) {
								...
								return { credentials: credentials || session.credentials };
							}).catch(function (err) {
