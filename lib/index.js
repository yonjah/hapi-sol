...
var idLength =  36,
	...
	uid =  (function (){
		...
		function get(counter, callback) {
			...
			counter++;
			if (counter > maxTries) {
				callback('too many tries');
			}
			try {
				id = crypt.randomBytes(idLength).toString('base64');
				if (store.indexOf(id)) { //retry
					get(counter, callback);
				} else {
					store.push(id);
					callback(null, id);
				}
				...
				setTimeout(uid.bind(null, counter, callback), 10);
			}
		}
...
Scheme.prototype.extend = function (request) {
	var self = this;
	function setNewId (session, callback, err, sessionId) {
		if (err) {
			callback(err);
		} else {
			request.setState(self.settings.cookie, sessionId);
			self.cache.set(sessionId, session, 0, callback);
		}
	}
	...
	request.auth.session = {
		set: function (session, callback) {
			...
			if (request.state.hasOwnProperty(self.settings.cookie)) {
				...
				if (typeof sessionId !== 'string') {
					...
					uid(setNewId.bind(this, session, callback));
				}
				// we have a session id already, so just fetch it and reuse it
			} else {
				sessionId = uid(setNewId.bind(this, session, callback));
			}
		},
		clear: function (callback) {
			...
			if (request.state.hasOwnProperty(self.settings.cookie)) {
				sessionId = request.state[self.settings.cookie];
				request.clearState(self.settings.cookie);
				self.cache.drop(sessionId, callback);
			}
		}
	};
};
