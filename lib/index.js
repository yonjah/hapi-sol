...
function implementation (server, options) {
	...
	server.ext('onPostAuth', (request, h) => {
		if (request.auth.error && !(request.auth.error instanceof AuthError)) {
		...
		async authenticate (request, h) {
			try {
				if (rlClient) {
					...
					if (!limit.conformant) {
						return unauthenticated(request, h, boom.tooManyRequests(), limit);
					}
				}
				return await validate(request, h);
			} catch (err) {
				let limit;
				if (rlClient && err instanceof AuthError && err.message !== 'Not authenticated') { // don't rate limit valid sessions that are not authenticated or unexpected errors
					try {
						const key = await settings.rlGetKey(request);
	...
	function unauthenticated (request, h, err, limit) {
		...
		if (appendNext) {
			...
			uri += appendNext + '=' + encodeURIComponent(request.url.path || request.url.pathname);
		}
